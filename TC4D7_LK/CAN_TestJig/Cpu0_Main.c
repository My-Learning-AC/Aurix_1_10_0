 ...
 *              are stored in Rx FIFO0 and Rx FIFO1 or dedicated Rx Buffer based on the filter configuration.
 *              Upon storing the messages, the interrupt service routine is called and the content of the received
 *              CAN message is read. Once the content of all the received messages is read, the received data is compared to
 *              the transmitted data. If all messages are received without any error detected, an LED is turned on to confirm
 *              successful message reception.
 *
 * \name CAN_Filtering_1_KIT_TC499_COM_TRB
 * \version V1.0.0
 * \board TriBoard TC499A COM, KIT_TC499_COM_TRB, TC499XE_AA-Step_CC_COM
 * \keywords CAN, MCMCAN, AURIX, INTERRUPT, LOOP-BACK, MCMCAN
 * \documents README.md
...
void core0_main(void)
{
    ...
    while(1)
    {
        IfxPort_togglePin(g_led.port, g_led.pinIndex);
        waitTime(IfxStm_getTicksFromMilliseconds(100u));
    }
}
