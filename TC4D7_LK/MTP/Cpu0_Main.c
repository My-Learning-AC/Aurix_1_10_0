...
#include "mtp\mtp_application_main.h"
#include "mtp\Silicon_Details.h"
#include "mtp\User_LED.h"
#include "mtp\User_Button.h"
#include "mtp\Data_Flash.h"
#include "mtp\GPIO_Test.h"
#include "mtp\Potentiometer.h"
#include "mtp\I2C_EEPROM.h"
#include "mtp\CAN.h"
#include "mtp\Gigabit Ethernet\Gigabit_Ethernet.h"
...
boolean MTP_Test_Procedure(enum MTP_Test_Cases test_case, enum MTP_Test_Components test_element, boolean (*test_func)(void), char *test_element_str, enum MTP_TestType test_type, boolean time_limit)
{
    ...
    printf("ins: Go for the next test\n");
...
void MTP_Test()
{
    boolean gpio = FALSE, led = FALSE, btn = FALSE, pot = FALSE, i2c_eeprom = FALSE, can = FALSE, ethernet = FALSE;
    ...
    while (!test_completed)
    {
        if (new_test)
        {
            ...
            switch (current_test)
            {
                case MTP_GPIO:
                    element = GPIO_E;
                    procedure_complete = MTP_Test_Procedure(current_test, element, Pin_Test, Test_Elements_str[element], MTP_AUTO, TRUE);
                    if (!procedure_complete)
                        continue;
                    gpio = procedure_complete;
                    new_test = FALSE;
                break;
            ...
            case MTP_LED:
                    init_LED();
                    element = LED_E;
                    procedure_complete = MTP_Test_Procedure(current_test, element, LED_Pttern, Test_Elements_str[element], MTP_MANUAL, FALSE);
                    if (!procedure_complete)
            ...
            case MTP_POT:
                    initTMADC_pot();
                    element = POT_E;
                    procedure_complete = MTP_Test_Procedure(current_test, element, Potentiometer_Test, Test_Elements_str[element], MTP_AUTO, TRUE);
                    if (!procedure_complete)
                        continue;
            ...
            case MTP_EEPROM:
                    init_I2C_module();
                    element = EEPROM_E;
                    procedure_complete = MTP_Test_Procedure(current_test, element, I2C_EEPROM_Test, Test_Elements_str[element], MTP_AUTO, TRUE);
                    if (!procedure_complete)
                        continue;
                    i2c_eeprom = procedure_complete;
                    new_test = FALSE;
                break;
            ...
            case MTP_CAN:
                    initMCMCAN();
                    element = CAN_E;
                    procedure_complete = MTP_Test_Procedure(current_test, element, transmitCanMessage, Test_Elements_str[element], MTP_AUTO, TRUE);
                    if (!procedure_complete)
                        continue;
                    can = procedure_complete;
                    new_test = FALSE;
                break;
            ...
            case MTP_ETHERNET:
                    Gig_Eth_Init_First();
                    element = ETHERNET_E;
                    procedure_complete = MTP_Test_Procedure(current_test, element, Gig_Eth_Send_Frame, Test_Elements_str[element], MTP_AUTO, TRUE);
                    if (!procedure_complete)
                        continue;
                    ethernet = procedure_complete;
                    new_test = FALSE;
                break;
            ...
            if(gpio && led && btn && pot && i2c_eeprom && can && ethernet)
            {
                test_completed = TRUE;
                printf("ins:\n\r --- All tests are completed --- \n\r");
            }
        }
    }
}
...
void core0_main(void)
{
    ...
    printf("log: KIT_A3G_TC4D7_LITE_V02 Manufacturing Test\n");
